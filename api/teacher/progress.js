...
function redirectWithFilter(response, filter) {
...
export function validateConfirmationForm(form, students) {
  ...
  return { studentId, zone, action, filter, confirmed: action === 'confirm', errors };
}
...
function renderZoneConfirmation(student, progressRows, links, filter) {
...
export default async function handler(request, response) {
  ...
  if (request.method === 'POST') {
    ...
    if (result.errors.length) {
      ...
      if (error) {
        ...
        return redirectWithFilter(response, filter);
      }
    }
  }
  ...
  const rows = students.map((s) => {
    ...
    return `<tr><td>${escapeHtml(s.display_name)}</td><td>${active ? escapeHtml(zoneLabels[active.zone] ?? active.zone) : 'Ninguna'}</td><td>${list.filter((p) => p.status === 'finished').length}</td><td>${confirmed} de ${Object.keys(zoneLabels).length}</td><td><div class="confirmation-list">${renderZoneConfirmation(s, list, links, filter)}</div></td></tr>`;
  }).join('');
  const messageHtml = message ? `<p class="${message.kind === 'error' ? 'teacher-error' : 'teacher-status'}">${escapeHtml(message.text)}</p>` : '';
  const body = `<section class="teacher-panel"><p>Fecha escolar: ${workDate}. El tiempo mostrado es <strong>tiempo de trabajo registrado</strong>, no prueba de finalización académica. Usa Confirmar completado solo después de revisar la plataforma o tarea correspondiente.</p>${messageHtml}<form><label>Filtro<select name="student" onchange="this.form.submit()">${options}</select></label><noscript><button class="teacher-button" type="submit">Ver</button></noscript></form>${rows ? `<table class="teacher-table"><thead><tr><th>Estudiante</th><th>Zona activa</th><th>Zonas terminadas</th><th>Confirmadas</th><th>Confirmación por zona y plataforma</th></tr></thead><tbody>${rows}</tbody></table>` : '<p>No hay estudiantes para mostrar.</p>'}</section>`;
